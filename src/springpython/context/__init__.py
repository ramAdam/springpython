...
class ApplicationContext(ObjectContainer):
    ...
    def __init__(self, config=None):
        ...
        post_processors = [
            ...
            if isinstance(object, ObjectPostProcessor)
        ]
        ...
        for obj_name, obj in self.objects.items():
            if not isinstance(obj, ObjectPostProcessor):
                for post_processor in post_processors:
                    self.objects[
                        obj_name
                    ] = post_processor.post_process_before_initialization(obj, obj_name)
        ...
        for obj_name, obj in self.objects.items():
            if not isinstance(obj, ObjectPostProcessor):
                for post_processor in post_processors:
                    self.objects[
                        obj_name
                    ] = post_processor.post_process_after_initialization(obj, obj_name)
        ...
        for obj_name, obj in self.objects.items():
            if isinstance(obj, DisposableObject):
                try:
                    ...
                    self.logger.error(
                        "Could not destroy object '%s', exception '%s'"
                        % (obj_name, format_exc())
                    )
                    ...
                    if callable(destroy_method):
                        try:
                            ...
                            destroy_method()
                            self.logger.debug(
                                "Successfully destroyed object '%s'" % obj_name
                            )
                        except Exception as e:
                            self.logger.error(
                                "Could not destroy object '%s', exception '%s'"
                        ...
                        self.logger.error(
                            "Could not destroy object '%s', "
                            "the 'destroy_method' attribute it defines is not callable, "
                            "its type is '%r', value is '%r'"
                            % (obj_name, type(destroy_method), destroy_method)
                        )

        self.logger.debug(
            "Successfully invoked the destroy_method on registered objects"
        )
